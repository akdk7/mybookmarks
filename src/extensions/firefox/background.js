...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch(request.action) {
    case 'setDavAuth':
      try { davAuth = request.auth || null; try { console.log('[DAV][BG] setDavAuth stored for host', (new URL(davAuth?.serverUrl||'')).hostname); } catch {}; sendResponse({ success: true }); }
...
async function handleDavRequest(req) {
  ...
  // Use XHR in Firefox too to preserve Authorization reliably
  return new Promise((resolve) => {
    try {
      ...
      xhr.onerror = function() { resolve({ ok: false, status: 0, statusText: 'Network Error', headers: {}, bodyText: '' }); };
...
async function handleFetchMetadata(url, options = {}) {
  try {
    ...
    // Fetch the page (prefer privileged XHR to avoid CORS limits)
    const text = await fetchTextPrivileged(url);
...
/**
 * Privileged text fetch using XMLHttpRequest to bypass page CORS
 */
function fetchTextPrivileged(url) {
...
chrome.runtime.onInstalled.addListener((details) => {
  ...
  try {
    chrome.storage.local.set({ enabled: true, autoFetchMetadata: true, cacheEnabled: true });
  } catch {}
});
// No chrome.action in Firefox MV2; skip browser action click handler here
...
// Force Authorization header for DAV endpoints
try {
